...
class Canvas:
    ...
    def hitsVertical(self,point):
        return round(point[0]) < 0 or round(point[0]) >= self._x

    def hitsHorizontal(self,point):
        return round(point[1]) < 0 or round(point[1]) >= self._y

    def hitsWall(self, point):
        return self.hitsVertical(point) or self.hitsHorizontal(point)

    def getReflection(self,point):
        return [-1 if self.hitsVertical(point) else 1, -1 if self.hitsHorizontal(point) else 1]
...
class TerminalScribe:
    ...
    def bounce(self,pos):
        ...
        # changes the direction to opposite 
